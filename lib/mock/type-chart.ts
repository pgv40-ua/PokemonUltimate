...
// Base type chart: chart[attacker][defender] = single-type effectiveness
// Values are 0 (immune), 0.5 (not very effective), 1 (normal), 2 (super effective)
// 0.25 and 4 are only possible when dual typing is applied via getDualEffectiveness
export const typeChart: Record<PokemonType, Record<PokemonType, Effectiveness>> = {
  normal: {
  ...
  poison: {
    ...
    rock: 0.5, ghost: 1, dragon: 1, dark: 1, steel: 0, fairy: 2,
  },
  ground: {
...
export function getDualEffectiveness(
  ...
  const result =
    defenders.length === 2
      ? typeChart[attacker][defenders[0]] * typeChart[attacker][defenders[1]]
      : typeChart[attacker][defenders[0]];
  // The product of two Effectiveness values is always a valid Effectiveness value (0/0.25/0.5/1/2/4)
